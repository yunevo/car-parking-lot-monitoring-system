...
#define MQTT_USERNAME "esp_in"
#define MQTT_PASSWORD "012345678"
#define MQTT_CLIENT_ID "client_esp_in"
...
/*display empty/taken state of the parking lot*/
void display_oled();
...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data) {
	...
	case MQTT_EVENT_CONNECTED:
		...
		// subcribe to topic providing notification to update parking lot map
	...
	case MQTT_EVENT_DATA:
		...
		// if there's a message from MAP topic 
		if (!strcmp(topic_name, MAP_TOPIC)) {
			// update the parking map
			xTaskCreate(display_oled, "oled", 2048, (void *)topic_data, 1, NULL);
		}
...
void init_rfid() {
	// pin configuration
	const rc522_start_args_t RFID_IN = { 
		.miso_io = MISO_PIN, 
		.mosi_io = MOSI_PIN, 
		.sck_io = SCK_PIN, 
		.sda_io = SDA_PIN, 
		.callback = &RFID_IN_handler,
...
void init_oled() {
	...
	xTaskCreate(display_oled, "oled", 2048, (void *)"0000", 1, NULL);
}


/*arg parameter store the state of lots in the parking lot
each charater from the start to the end of the array represents the state of each lot
with the value '0' meaning empty and the value '1' meaning taken*/
void display_oled(void *arg) {
	char *map_recei = (char *) arg;
